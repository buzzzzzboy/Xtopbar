...
| 鼠标离开条和顶部区域 0.4 秒 | 自动隐藏（可改 0.4 / 0.8 / 1.5 / 3 / 4 秒或常驻） |
...
| **刘海模式**：设置 → 悬浮条 →「刘海区触发调度中心」 | TopTab 悬浮条与预览**全部停用**，鼠标移到屏幕顶部中央（刘海下方）自动触发一次**调度中心**（合成键码 160，与真实 F3 完全等效），触发后指针自动瞬移到屏幕中央；离开区域停顿 0.3s 且距上次触发 ≥1.5s 后重新武装。菜单栏图标保留，点它可关掉此模式 |
...
设置窗口（开机自启 / 菜单栏图标 / 预览开关 / 自动隐藏时长 / 液态玻璃材质与不透明度 / 权限状态）：
...
- 位置：主屏可见区域顶部居中，距菜单栏 6pt
- 宽度：随 App 数量自适应，上限为屏宽 −24pt（超出可横向滚动）
- 透明度：待机 86%（可在设置里调 40%–100%），鼠标悬停 100%
- 分隔线：**每两个标签之间都是同一条深竖线**（1×18pt，opacity 0.22）
...
|---|---|
| 通用 | 开机自动启动、在菜单栏显示图标 |
| 悬浮条 | 启用悬浮条、窗口预览、只显示已打开的窗口、自动隐藏时长、背景材质、待机不透明度、进出场动画 |
